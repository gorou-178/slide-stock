...
### PM（設計タスク — 並列実行可）
  - [x] @pm T-750 — [設計] Cookieセキュリティ強化設計（__Host- prefix付与、Path見直し、Max-Age環境変数化）※実装済み確認 2026-06-28
  - [x] @pm T-752 — [設計] Google OAuthエンドポイントバージョン統一設計（v2/v3混在修正、scope調査）※各URLはGoogle Discovery Documentの正規エンドポイントであり対応不要と判断 2026-06-28
  - [x] @pm T-754 — [設計] DBスキーマ型最適化設計（全TEXT型見直し、statusカラム削除）※実装済み確認 2026-06-28
  - [x] @pm T-756 — [設計] UUID v7導入+publicId検討（crypto.randomUUID()→v7、外部公開ID要否）※実装済み確認 2026-06-28
  - [x] @pm T-758 — [設計] メモ機能改善設計（上限200文字、改行LF統一、前後トリム）※実装済み確認 2026-06-28

### DEV（実装タスク — PM設計完了後に実行）
  - [x] @dev T-751 — [実装] Cookieセキュリティ強化 [dep: T-750] ※実装済み確認 2026-06-28
  - [x] @dev T-753 — [実装] Google OAuthエンドポイント修正 [dep: T-752] ※対応不要と判断 2026-06-28
  - [x] @dev T-755 — [実装] stocksテーブルstatusカラム削除 [dep: T-754] ※実装済み確認 2026-06-28
  - [x] @dev T-757 — [実装] UUID v7導入 [dep: T-756] ※実装済み確認 2026-06-28
  - [x] @dev T-759 — [実装] メモ機能改善 [dep: T-758] ※実装済み確認 2026-06-28
...
=== Phase 6E: 本番デプロイ不具合修正 ===
  ...
  - [x] @qa T-812 — 本番デプロイ後の動作検証（T-810, T-811 修正後に実施。①Worker デプロイ成功 ②/api/health が {"status":"ok"} を返す ③ログインボタン押下で Google OAuth 画面に遷移 ④ログインページでローディングが消えてボタンが表示される）[dep: T-810, T-811] ※本番確認済み 2026-06-28


=== Phase 8: UI/レイアウト改善 ===
  - [ ] @dev T-820 — スライド一覧ページのPC向け3カラムレイアウト対応（/stocks 一覧のカードグリッドをPC表示時に3カラムで表示。レスポンシブ: モバイル1カラム→タブレット2カラム→PC3カラム）
...
--- 優先度4以降: 検討段階 ---
  ...
  - [x] @pm T-720 — [検討] 認証委譲: カスタムGoogle OIDC実装→Cloudflare Accessへの移行検討 → ADR-008 にて Rejected（コスト対効果低・ローカル開発悪化・ベンダーロックイン深化。再検討条件: マルチユーザー化/追加IdP/セキュリティインシデント）
  - [-] @pm T-730 — ~~[検討] ルーターフレームワーク導入~~ → SSR統合(T-701)完了によりAstro API Routes採用済み。不要
