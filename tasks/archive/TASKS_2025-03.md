...
### PM（設計タスク — 並列実行可）
  - [ ] @pm T-750 — [設計] Cookieセキュリティ強化設計（__Host- prefix付与、Path見直し、Max-Age環境変数化）
  - [ ] @pm T-752 — [設計] Google OAuthエンドポイントバージョン統一設計（v2/v3混在修正、scope調査）
  - [ ] @pm T-754 — [設計] DBスキーマ型最適化設計（全TEXT型見直し、statusカラム削除）
  - [ ] @pm T-756 — [設計] UUID v7導入+publicId検討（crypto.randomUUID()→v7、外部公開ID要否）
  - [ ] @pm T-758 — [設計] メモ機能改善設計（上限200文字、改行LF統一、前後トリム）

### DEV（実装タスク — PM設計完了後に実行）
  - [ ] @dev T-751 — [実装] Cookieセキュリティ強化 [dep: T-750]
  - [ ] @dev T-753 — [実装] Google OAuthエンドポイント修正 [dep: T-752]
  - [ ] @dev T-755 — [実装] stocksテーブルstatusカラム削除 [dep: T-754]
  - [ ] @dev T-757 — [実装] UUID v7導入 [dep: T-756]
  - [ ] @dev T-759 — [実装] メモ機能改善 [dep: T-758]
