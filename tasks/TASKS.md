...
--- 優先度1〜2: 実施判断対象 ---
  ...
  - [x] @dev T-711 — [実装] Queue廃止+同期取得+フロント簡素化（T-710 設計 ADR-004 に基づく5フェーズ移行）。Phase1: handleCreateStock内でoEmbed同期fetch、Queue送信削除、stocks.test.ts/integration.test.ts更新 → Phase2: queue-consumer/worker-index/queue.ts削除、security-verification.test.ts更新 → Phase3: wrangler.toml/types/env.d.ts/package.json整理 → Phase4: フロント簡素化（ポーリング~50行削除、pending/failed分岐削除、CSS整理） → Phase5: E2E検証。各フェーズでテストGREEN維持 [dep: T-710]
  - [x] @qa T-712 — [検証] Queue廃止+同期取得後のリグレッションテスト（①UT全通過（174/174 GREEN） ②E2E全通過（21/21 GREEN） ③POST /api/stocks が即座にメタデータ込みで返ること ④ポーリング・pending表示が完全に除去されていること）[dep: T-711]
