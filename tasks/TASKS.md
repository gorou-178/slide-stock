...
- [x] @pm T-500 — Google OIDC認証仕様策定（コールバックURL、セッション形式・有効期限、ログアウトフロー）
- [x] @pm T-510 — プロバイダ検出仕様策定（各プロバイダのURLパターン、正規化ルール、バリデーションエラー定義）
- [x] @pm T-520 — oEmbed/Queue処理仕様策定（各プロバイダのoEmbedエンドポイント、Queueメッセージスキーマ、リトライ・失敗ポリシー）
- [x] @pm T-530 — Stock API仕様策定（リクエスト/レスポンススキーマ、ページネーション方式、重複URL対応、エラーレスポンス形式）
- [x] @pm T-540 — Memo API仕様策定（リクエスト/レスポンス、upsert動作、最大文字数、空メモ対応）
- [x] @pm T-550 — フロントエンドUI仕様策定（コンポーネント階層、画面レイアウト、エラー/ローディング/空状態の定義）

### DEV — 基盤整備（依存なし・並列実行可）
- [x] @dev T-535 — worker/index.ts ルーティングリファクタ（現在のif/elseからハンドラマップへ。パスパラメータ対応）
- [x] @dev T-570 — APIエラーハンドリング統一 & CORS設定（Pages↔Workers間のクロスオリジン対応 or 同一オリジン化）
- [x] @dev T-506 — 環境変数・Secrets設定（GOOGLE_CLIENT_ID, SESSION_SECRET を wrangler.toml / .dev.vars に追加）[dep: T-500]
