...
**return_to クエリパラメータ:**
- セッション切れで `redirectToLogin()` 経由でこのページに飛ばされた場合、`?return_to=<元のパス>` が付与される（spec §6.2）
- 値はクライアント側で「`/` で始まり `//` で始まらない相対パス」のみ採用、それ以外は無視する（オープンリダイレクト対策）
- 有効な値の場合、`Google でログイン` リンクの href も `/api/auth/login?return_to=<エンコード値>` に書き換える。サーバー側で再検証してから `__Host-auth_return_to` Cookie に保存する（auth-spec.md §3.1）
...
ストックは登録時点で oEmbed 取得が完了しているため、すべてのカードは `title` / `embed_url` が揃った状態で表示される（`author_name` は Google Slides では仕様上 `null`）。`pending` / `failed` 状態は ADR-009 §4-2 の rollback semantics により存在しない。Google Slides の HTML タイトル取得失敗もメタデータ null を残さない hard failure として扱う（ADR-009 §4-5、軟性失敗の廃止）。
