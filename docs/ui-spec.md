...
> **設計判断（`status` 廃止）:** `stocks.status` カラムは migration 0003 で削除済み（ADR-009 §4-3 でも維持）。同期モデル + rollback semantics により取得失敗時に stock を作成しないため、`pending` / `failed` カードは UI に存在しない。クライアントはメタデータの有無を `embed_url` / `title` で判定する。
