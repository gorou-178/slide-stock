...
3. POST /api/stocks { url: inputValue }
   ※ 同期モデル: サーバーは stock INSERT + best-effort oEmbed 取得を行ってから 201 を返す
4. 3 秒経過後にフォーム下に「スライド情報を取得しています...」と進捗テキスト表示（aria-live="polite"）
5. 成功（201、メタデータ充足）→ 完成済みのストックカード（title, author_name, embed_url 等が揃った状態）を一覧の先頭に追加、フォームをリセット
6. 成功（201、メタデータ null）→ フォールバックカード（元 URL のリンク + provider バッジ、embed プレビューなし）を一覧の先頭に追加、フォームをリセット
7. エラー（400 INVALID_URL / UNSUPPORTED_PROVIDER / INVALID_FORMAT / UNSUPPORTED_URL_TYPE）→ #url-error にエラーメッセージ（API の error フィールド）を表示
8. エラー（409 DUPLICATE）→ 「このスライドは既にストック済みです」を表示
9. エラー（401）→ /login にリダイレクト
10. クライアント側タイムアウト 15 秒（サーバー側 fetch 10 秒 + バッファ）で「タイムアウトしました。もう一度お試しください。」を表示
11. 送信ボタンを元に戻す
```

> **設計判断（同期モデル: optimistic insert + best-effort 取得）:** `POST /api/stocks` は重複チェック通過後にまず stock を INSERT し、その直後にプロバイダの oEmbed 取得を best-effort で実行する。取得成功時は UPDATE でメタデータを反映、失敗時は stock をそのまま残してメタデータ null の 201 を返す。元 URL 自体に「あとで開けばよい」価値があるため、メタデータ未取得を 5xx 化しない。
> リトライは行わず、1 試行 10 秒のタイムアウトのみ。`pending` / `failed` 状態と `status` カラムは ADR-004 / migration 0003 で削除済み。詳細は ADR-004、oembed-spec.md §5 / §6 / §7、stock-api-spec.md §3。
...
ストックは `embed_url` の有無で 2 通りの表示を出し分ける。`pending` / `failed` 状態は ADR-004 / migration 0003 で廃止されており、`status` カラム自体が存在しない。

| 条件 | 表示 |
|------|------|
| `embed_url IS NOT NULL` | 通常カード: タイトル（`title || 'タイトルなし'`）+ 著者 + provider バッジ + embed プレビュー + メモ抜粋 |
| `embed_url IS NULL` | フォールバックカード: 「タイトル」位置に `original_url` のリンクを表示 + provider バッジ。embed プレビューと著者は出さない。メモ抜粋は通常通り |

> **設計判断（メタデータなしカードの扱い）:** `POST /api/stocks` は元 URL を価値ある情報として扱い、oEmbed 取得失敗時も 201 で stock を返す（ADR-004、stock-api-spec.md §3.6）。UI 側はメタデータ null のカードを「破損」ではなく「タイトル未取得のリンク集」として扱い、ユーザーが元 URL を直接開けば本来の用途は果たせる。再取得 UI は MVP では未実装（oembed-spec.md §7.3）、ユーザーは DELETE → 再 POST で再試行する。
...
> **同期モデル（ADR-004）下での `embed_url === null` の扱い:** `pending` / `failed` 状態と `status` カラムは廃止されているが、`embed_url === null` のレコードは存在し得る（プロバイダ取得失敗・SpeakerDeck/Docswell の oEmbed 異常レスポンス・Docswell の embed URL がドメイン外と判定されたケースなど、oembed-spec.md §6.3）。詳細画面ではその場合のみ「スライドの読み込みができません」プレースホルダーと `original_url` を直接開くリンクを表示する。
...
> **設計判断（同期モデル化、ADR-004）:** 旧仕様の「pending ストックのポーリング」は廃止済み。POST /api/stocks のレスポンスを待つ間のクライアント挙動は §5.3.1 の送信処理 4-11 を参照。
...
サーバー側タイムアウトは oembed-spec.md §8 の **10 秒**（プロバイダ呼び出し 1 試行）。クライアント側 15 秒はサーバー応答に余裕を持たせるための上限（fetch 10 秒 + バッファ）。oEmbed 取得が失敗しても 201 が返るため、通常はクライアント側タイムアウトに達する前にレスポンスが返る。
...
| 重複登録（409 DUPLICATE_STOCK） | 「このスライドは既にストック済みです」 | フォーム下の `#url-error` |
| クライアント側タイムアウト（15 秒） | 「タイムアウトしました。もう一度お試しください。」入力値はフォームに保持 | フォーム下の `#url-error` |
| ネットワークエラー（fetch 例外） | 「サーバーに接続できません。ネットワーク接続を確認してください。」入力値はフォームに保持 | フォーム下の `#url-error` |
...
> **note:** プロバイダ側の oEmbed 取得失敗（404 / 403 / 5xx / タイムアウト）は API としてはエラー扱いではなく、201 + メタデータ null の stock として返る（ADR-004、oembed-spec.md §6.3）。クライアントは §5.3.3 のフォールバックカードを表示するだけで、エラーメッセージは出さない。
